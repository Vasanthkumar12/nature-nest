  ...
  return (
    <>
      ...
      <div className="flex justify-center mb-10 px-4">
        <div className="bg-white flex items-center rounded-full overflow-hidden shadow-md w-full max-w-md">
          <FaSearch
          ...
          <input
            ...
            className="p-2 pr-4 w-full text-lime-600 outline-none"
          />
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6 px-4 pb-10">
        {result.map((product) => (
          <div
            key={product.id}
            className="bg-white w-full max-w-xs mx-auto tracking-wide p-5 flex flex-col justify-evenly overflow-hidden border border-gray-200 rounded-lg shadow-md"
          >
            ...
            <Link to={`/view-details/${product.id}`}>
              <button className="mt-2 p-2 pt-1 text-lime-600 text-2xl bg-lime-200 border-2 border-lime-200 font-bold hover:bg-lime-500 hover:text-lime-200 rounded-md cursor-pointer w-full">
                Add to Cart
              </button>
            </Link>
          </div>
